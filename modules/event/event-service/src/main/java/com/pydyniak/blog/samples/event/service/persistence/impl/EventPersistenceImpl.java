...
import com.pydyniak.blog.samples.event.service.persistence.EventUtil;
import com.pydyniak.blog.samples.event.service.persistence.impl.constants.EventPersistenceConstants;
...
public class EventPersistenceImpl
	...
	public Event[] findByG_S_PrevAndNext(
		...
		try {
			...
			array[0] = getByG_S_PrevAndNext(
				session, event, groupId, status, orderByComparator, true);

			array[1] = event;

			array[2] = getByG_S_PrevAndNext(
				session, event, groupId, status, orderByComparator, false);
	...
	protected Event getByG_S_PrevAndNext(
		Session session, Event event, long groupId, int status,
		OrderByComparator<Event> orderByComparator, boolean previous) {
	 ...
	 */
	@Override
	public List<Event> filterFindByG_S(
		long groupId, int status, int start, int end) {
	...
	protected Event filterGetByG_S_PrevAndNext(
		Session session, Event event, long groupId, int status,
		OrderByComparator<Event> orderByComparator, boolean previous) {
	...
	private static final String _FILTER_SQL_SELECT_EVENT_WHERE =
		"SELECT DISTINCT {event.*} FROM Event_Event event WHERE ";
	...
	private static final String _FILTER_SQL_COUNT_EVENT_WHERE =
		"SELECT COUNT(DISTINCT event.eventId) AS COUNT_VALUE FROM Event_Event event WHERE ";
